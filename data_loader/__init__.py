...
def import_transforms():
    ...
    if transforms_kitti is None or transforms_cityscapes is None:
        raise ValueError(
            "No transforms method was found in {}.py,".format(transforms_filename)
        )
...
def prepare_dataloader(args, data_mode, verbose=True):
    ...
    if args.dataset == "kitti":
        ...
        dataset_kitti = StreetLoader(
            args.data_dir,
            data_paths_file_kitti,
            data_mode,
            args.train_ratio,
            transform=data_transform_kitti,
        )
        dataset_cityscapes = StreetLoader(
            args.data_dir,
            data_paths_file_cityscapes,
            data_mode,
            args.train_ratio,
            transform=data_transform_cityscapes,
        )
